...
PF <- function(pars, C, data, u, ndays, npart = 10, MD = TRUE, a1 = 0.01, a2 = 0.2, b = 0.1, a_dis = 0.05, b_dis = 0.01, saveAll = NA, PF = TRUE, ncores = detectCores()) {
  ...
  runs <- mclapply(1:nrow(pars), function(k, pars, C, u, npart, ndays, data, MD, a1, a2, b, a_dis, b_dis, saveAll, PF) {
    ...
    if(PF) {
      data1 <- select(data, t, (starts_with("H")) & ends_with("obs")) 
      
      data2 <- select(data, t, (starts_with("DI") | starts_with("DH")) & ends_with("obs")) %>%
        {rbind(rep(0, ncol(.)), .)} %>%
        mutate(across(!t, ~. - lag(.))) %>%
        slice(-1) %>%
        as.matrix()
      
      data <- cbind(data2, select(data1, -t))%>%
        as.matrix()
    ...
    for(t in 1:ndays) {
      ...
      for(i in 1:npart) {
        ...
        if(PF) {
          ...
          obsInc <- data[t, -1]
          obsDiffs <- obsInc - c(DIinc, DHinc, disSims[[i]][10, ])
