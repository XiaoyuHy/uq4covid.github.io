...
while(n_inbatch*batch <= Nruns){
  ...
  sims_mdCumD <- map_dfr(runs_md$particles, function(e) {map(map(e,2),  ~map(., ~as.vector(t(.)))) %>%
      map(~do.call("rbind", .)) %>%
      map(as_tibble) %>%
      bind_rows(.id = "t")}, .id= "run") %>%
    mutate(run= (batch-1)*n_inbatch + as.numeric(run))
...
# write.csv(tmp3, 'cum_death_hPrev.csv', row.names = FALSE)
age_label <- c("<5", "5-17", 
...
p <-  dplyr::select(sims_md, -starts_with(notPlot)) %>%
  ...
  geom_line(
    ...
    data = pivot_longer(select(data, t, ends_with("obs")) %>% select(-starts_with(notPlot))%>% select(!ends_with("cumobs")) %>% filter(t <= pf_days+predict_days), !t, names_to = "var", values_to = "n") %>%
...
# p
ggsave('perfectModel_D_H.pdf', p, width = 10)
