...
PF <- function(pars, C, data, u, ndays, npart = 10, MD = TRUE, a1 = 0.01, a2 = 0.2, b = 0.1, a_dis = 0.05, b_dis = 0.01, saveAll = NA, PF = TRUE, ncores = detectCores()) {
    ...
    for(t in 1:ndays) {
      ...
      for(i in 1:npart) {
        ...
        if(PF) {
          ...
          Dinc = DIinc + DHinc
          obsDiffs_D <- obsInc_D - Dinc
          obsDiffs_H <- obsPre_H - sum(disSims[[i]][10, ])
          obsDiffs <- c(obsDiffs_D, obsDiffs_H)
          sum_dens <- sum(ldtskellam_n_cpp(obsDiffs, a1 + b * c(Dinc,sum(disSims[[i]][10, ])), a2 + b * c(Dinc,sum(disSims[[i]][10, ])), LB = -c(Dinc,sum(disSims[[i]][10, ])), UB = obsInc)) 
          
          if (is.finite(sum_dens)){
            weights[i] <- weights[i] +  sum_dens
          }else{
            mu = a1 + b * c(Dinc,sum(disSims[[i]][10, ])) - (a2 + b * c(Dinc,sum(disSims[[i]][10, ])))
            std = sqrt(a1 + b * c(Dinc,sum(disSims[[i]][10, ])) + (a2 + b * c(Dinc,sum(disSims[[i]][10, ]))))
            sum_dens <- sum(ldtnorm(obsDiffs, mu, std,  LB = -c(Dinc,sum(disSims[[i]][10, ])), UB = obsInc))
