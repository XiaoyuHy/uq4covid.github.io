...
PF_sim <- function(pars, C, data, u, ndays,simdays = 0, npart = 10, MD = TRUE, a1 = 0.01, a2 = 0.2, b = 0.1, a_dis = 0.05, b_dis = 0.05, saveAll = NA, PF = TRUE, ncores = detectCores()) {
  ...
  runs <- mclapply(1:nrow(pars), function(k, pars, C, u, npart, ndays, simdays, data, MD, a1, a2, b, a_dis, b_dis, saveAll, PF) {
    ...
    if(PF) {
      ...
      dataD <- select(dataD, t, (starts_with("D")) & ends_with("obs"))  %>%
        as.matrix()
      mode(dataD) <- "integer"
      
      dataH <- select(dataH, t, (starts_with("H")) & ends_with("obs"))  %>%
        as.matrix()
      mode(dataH) <- "integer"
    }
    ...
    for(t in 1:(ndays+simdays)) {
      ...
      for(i in 1:npart) {
        ...
        disSimsIncD[[i]] <- c(DIinc, DHinc)
        disSimsH[[i]] <- disSims[[i]][10, ] 
        if(PF[t]) {
          ## calculate log observation error weights
          obsInc_D <- dataD[t, -1]
          obsPre_H <- dataH[t, -1]
          obsInc <- c(obsInc_D, obsPre_H)

          obsDiffs_D <- obsInc_D -c(DIinc, DHinc)
          obsDiffs_H <- obsPre_H - disSims[[i]][10, ]
          obsDiffs <- c(obsDiffs_D,obsDiffs_H)
          sum_dens <- sum(ldtskellam_n_cpp(obsDiffs, a1 + b * c(DIinc, DHinc, disSims[[i]][10, ]), a2 + b * c(DIinc, DHinc, disSims[[i]][10, ]), LB = -c(DIinc, DHinc, disSims[[i]][10, ]), UB = obsInc)) 
          ...
          if (is.finite(sum_dens)){
            ...
            oe_h <- rtskellam_n_cpp(a1 + b * disSims[[i]][10, ], a2 + b * disSims[[i]][10, ], LB = -disSims[[i]][10, ], UB = obsPre_H)
            disSimsH[[i]] <- disSimsH[[i]] + oe_h
            disSimsCumD[[i]] <- disSimsIncD[[i]] + disSimsCumD[[i]]
            
          }else{
            mu = a1 + b * c(DIinc, DHinc, disSims[[i]][10, ]) - (a2 + b * c(DIinc, DHinc,disSims[[i]][10, ]))
            std = sqrt(a1 + b * c(DIinc, DHinc,disSims[[i]][10, ]) + (a2 + b * c(DIinc, DHinc,disSims[[i]][10, ])))
            sum_dens <- sum(ldtnorm(obsDiffs, mu, std,  LB = -c(DIinc, DHinc, disSims[[i]][10, ]), UB = obsInc))
            ...
            mu2 = a1 + b * disSims[[i]][10, ] - (a2 + b * disSims[[i]][10, ])
            std2 = sqrt(a1 + b * disSims[[i]][10, ] + (a2 + b * disSims[[i]][10, ]))
            oe_h <- round(rtnorm(round((-disSims[[i]][10, ] - mu2)/std2), round((obsPre_H - mu2)/std2))*std2 + mu2)
            disSimsH[[i]] <- disSimsH[[i]] + oe_h
            disSimsCumD[[i]] <- disSimsIncD[[i]] + disSimsCumD[[i]]
      ...
      if(!is.na(saveAll)) {
        if(saveAll) {
          out[[t]] <- list(counts =disSims, cumD =disSimsCumD, hPrev = disSimsH)
        } else {
